<x-dashbar>
    ...
    <div class="nk-block">
        <form class="column g-3 needs-validation" action="{{ route('articles.store') }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            <div class="col-md-4 mb-4 position-relative">
                <label for="title" class="form-label">Judul</label>
                <input type="text" name="title" class="form-control @error('title') is-invalid @enderror"
                    id="validationTooltip02" value="{{ old('title') }}" required>
                @error('title')
            ...
            <div class="col-md-4 mb-4 position-relative">
                ...
                <textarea name="body" rows="5" class="form-control @error('body') is-invalid @enderror"
                    placeholder="Masukkan konten artikel">{{ old('body') }}</textarea>
                @error('body')
                    <div class="alert alert-danger mt-2">
                        {{ $message }}
                    </div>
