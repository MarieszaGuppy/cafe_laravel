<x-dashbar>
    <x-slot:title><button onclick="location.href='{{ route('galleries.index') }}'"
            class=" btn btn-primary d-md-inline-flex"><i class="ri-arrow-left-line"></i></button> Ubah Galeri</x-slot:title>
    <x-slot:indexx></x-slot:indexx>
    <x-slot:desc></x-slot:desc>
    <x-slot:heading></x-slot:heading>

    <div class="nk-block">
        <form class="column g-3 needs-validation" action="{{ route('galleries.update', $gallery->id) }}" method="POST"
            ...
            <div class="col-md-4 mb-4 position-relative">
                ...
                <input type="text" name="slug" class="form-control @error('slug') is-invalid @enderror"
                    id="validationTooltip02" value="{{ old('slug', $gallery->slug) }}" required>
                @error('slug')
                    <div class="alert alert-danger mt-2">
                        {{ $message }}
                    </div>
